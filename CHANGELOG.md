...
- Author edit pages now also show linked books that need review or have a missing directory, matching the deletion safeguard instead of misleadingly displaying zero books.
- Roles are now groups of permissions (user rebuild phase 3): new `roles` and `role_permission` tables (purely additive migrations) bundle permissions per `users.role` key, and `User::hasPermission()` resolves the user's role bundle in addition to direct per-user grants. The ordinary user roles (`user`, `library-user`, `hybrid-user`) imply the standard library-management permissions (books, authors, genres, series, tags, badges) so a normal account can use the management surfaces out of the box with no per-user checkbox configuration; admin roles imply everything (the admin bypass remains as a safety net for permission keys added after seeding); the lifecycle `unverified` role and the pure-listener `librivox-user` role imply nothing. The canonical role keys, labels, and bundles live on the `Role` model and are seeded by migration; admin/web role forms and API role validation are driven from that single source, the admin user edit page annotates which permissions come "from role", and `GET /api/v1/user` / `GET /api/v1/me` now also return `role` plus the effective `permissions` array. The management branch of the unified book list now also passes the requesting user's id, so per-user tag ban/require and block rules keep applying to the management view layer now that ordinary roles hold `manage-books`.
- Book detail pages now resolve the session's `DocumentstoreUser` wrapper to its persisted `User` model before loading scoped book tags, preventing authenticated book views from failing with a type error.
- The shared book list's AJAX renderer now includes each book's duration in the Length column, matching the initial server-rendered list.
- The shared `/books` template now also preserves the management-list capabilities formerly isolated at `/admin/books`: full administrators retain the AI Library Assistant, `manage-books` holders can select every legacy sort order, see paginated result counts, and the AJAX list applies the selected management sort rather than reverting it after page load. Old `/admin/books` bookmarks continue to redirect to this shared surface.
- Admin user editing now includes comma-separated Required Tags and Ignored Tags. These locked filters are applied to every book listing for that user, while preserving filters the user set for themselves.
...
- Book search (web book list, `GET /books`, `GET /books/search`) now supports inline name-based search tokens (`genre:Fantasy`, `author:"Brandon Sanderson"`, `series:Name`, `tag:Name`) alongside the existing id-based tokens (`genreId:N`, `authorId:N`, `seriesId:N`, `bookId:N`); an opt-in `semantic=true` "smart search" that ranks results by AI semantic similarity to the query text instead of exact/partial text match, reusing the existing recommendation-engine embedding pipeline (falls back to normal search when no embedding provider is configured); and tag filtering on the web book list's AJAX endpoint (`/api/books/json`), which previously had no tag support at all and, as a related fix, now also enforces per-user tag ban/require rules there (previously silently skipped on that endpoint only).
...
- Genre detail sub-lists: `GET /genres/{genre}/books` (sortable `recent`/`alpha`/`random`) and `GET /genres/{genre}/series` (sortable `alpha`/`recent`/`length`, i.e. book count in that genre) join the existing `GET /genres/{genre}/authors`, which also gained a `sort=random` option. `MySqlService::listBooks()` gained a `random` sort case.
- `GET /authors` and `GET /authors/{authorId}` now return a real `image_url` (one of the author's book covers) instead of always `null`, plus a new `sample_book_titles` (up to 3 random titles by that author).
- Per-user content filter: `GET/POST/DELETE /users/me/tag-filters` lets a user require or ban a tag (system/group/own-scope tags all count) across every book listing/search/discovery-shelf surface. `GET/POST/DELETE /admin/users/{id}/tag-filters` lets an admin set a locked rule on another user's account that the user themselves cannot change or remove. Applied unconditionally inside `MySqlService::listBooks()` (not gated by an explicit `?tag=` param), so it covers every listing surface for free.
...
- Added `books:detect-chapters` to detect embedded audio chapters with `ffprobe`, store them in the `chapters` table, and write them into `librarian.json`; normal JSON regeneration reads chapters from the database, imports JSON-only chapters into the database when needed, and does not run detection.
- `books:cache-file-chunk-hashes` and `books:detect-chapters` now accept inclusive book-id ranges via `--book=100-150`.
...
- The admin "Show QR code" login QR (`POST /api/v1/admin/users/{id}/login-qr`, web-admin `generateLoginQr`) now encodes the account's username and the plaintext 6-digit one-time code as query parameters on the magic-link URL (`.../auth/magic/<token>?username=<u>&otp=<code>`) and returns `server_name`/`api_url`/`username`/`email`/`code`/`token` alongside the URL. A regular camera scanner still reaches the magic landing page and deep-links into the app, and the Librarian app's connect screen can now parse the same QR and redeem the token via `POST /api/v1/auth/otp/verify` with no password or email step. The web modal also displays the server, username, code and expiry under the QR.
...
- `POST /api/v1/pending-downloads` returned a 500 (caught only by the log) whenever the AudiobookBay bridge extension re-registered an already-known magnet, violating the `magnet_infohash` unique index — and the extension retried forever. Registration is now idempotent: a known infohash refreshes/requeues the existing record (unless already consumed, which is returned untouched), and concurrent duplicate inserts resolve to the winner.
...
- `StatisticsController::getOverview()`'s remaining fields — `total_listening_time_ms`, `books_started`, `average_session_duration_ms`, `favorite_genres`, `daily_stats`, and `listening_minutes` — still read the dead `listening_statistics` table even after the streak/badge migration above, so `GET /statistics/overview` kept returning zeros/empty arrays for everything except `books_finished` and the two streak fields. Migrated the rest onto `ListeningActivityService` as well.
- `ListeningActivityService::getLongestStreak()` compared Carbon 3's `diffInDays()` result against the literal int `1` to detect consecutive days, but that method now returns a signed float (`$a->diffInDays($b)` is `$b - $a`), which came back negative for ascending-sorted date pairs — so no pair was ever recognized as consecutive and `longest_streak` was pinned at 1 regardless of real activity, even while `current_streak` (which uses a different, correctly-signed check) reported the true value. Fixed by comparing the absolute day count. Also hardened an adjacent `diffInDays()` call in `BadgeService::getQuickFinishCount()` the same way, since it depends on the same sign convention.
- Current-streak calculation required activity on the literal current calendar day or returned 0 immediately — a real multi-day streak reported as 0 whenever checked before that day's first listening session. Now allows a one-day grace period (a streak ending yesterday still counts as current).
- `books_finished` (statistics overview) and `books_completed` (badge criteria, including the "First Finish" badge) only counted completions from the legacy `book_progress`/`user_book_status` tables. The modern event-sourced finish path (`BOOK_FINISH` → `PositionMaterializer`) writes to `book_positions` instead, so real finishes on current app versions were invisible to both. Both now union completions from `book_positions` too.
- `BadgeService::getLibrarySize()` (the "N in Library" badge tier) only counted books with an explicit status tag (queue/wishlist/completed/etc. in `user_book_status`) — a user with a large scanned-in local library who never explicitly statused any of it would never earn this badge despite a real, large library. Now also counts any book with a `book_positions` row (written for virtually every playback interaction). Note: books that exist only in a local folder scan and have never been played still can't be counted server-side — there's no sync signal for "this book exists in my library" independent of interaction or status; see `todo` for the follow-up.
- `StatisticsController::getDailyStatsOpenApi()` (`GET /statistics/daily`) and `getTimelineStats()`/`getTimelinePeriodDetails()` (`GET /statistics/timeline`) — both actively called by current clients — also read the dead `listening_statistics` table; migrated onto `ListeningActivityService` alongside `getOverview()`. This also collapsed the MySQL/SQLite-specific raw-SQL day-of-week and period-grouping branches into portable Carbon-based logic.
- `sessions:close-orphaned` (the scheduled command that synthesizes a `SESSION_END` for a `SESSION_START` with no matching end, e.g. after a client crash) checked for an existing end event by `device_id === $start->device_id` — but its own synthesized events are written with `device_id = 'server-synthesized'`, which never matches. Every rerun therefore treated the same orphan as still unclosed and synthesized another duplicate `SESSION_END`, unbounded — found in production as a single orphaned session accumulating 96–145 duplicate end events, inflating some users' reported daily listening time past 24 hours. Now also recognizes its own prior synthetic output as closing the orphan. Also fixed the synthesized event's metadata to use the same `sessionDurationMs`/`adjustedDurationMs` camelCase keys real client events use (it was writing `session_duration_ms`, which nothing reads, silently zeroing out the very listening time it was trying to preserve).
- `ListeningActivityService::getSessions()` now deduplicates `SESSION_END` events sharing `(user_id, book_id, device_id, timestamp_ms)` before summing — the server has no way to detect a resynced duplicate at write time (each retry gets a fresh client-generated UUID `id`, its only true dedup key), so without this, any repeated sync of one real session inflated listening totals arbitrarily. This was the direct cause of the >24h/day totals described above, independent of the `sessions:close-orphaned` bug — both real client resyncs and the orphan-closer could produce the same class of duplicate.
