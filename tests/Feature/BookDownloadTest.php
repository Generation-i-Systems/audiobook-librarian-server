...
class BookDownloadTest extends TestCase
{
    ...
    public function test_user_can_download_book_bundle()
    {
        ...
        $book = Book::factory()->create([
            'directory_path' => 'books/test_book',
        ]);

        // Create directory
        Storage::fake('public');  // use fake storage
        Storage::disk('public')->makeDirectory($book->directory_path);
        Storage::disk('public')->put($book->directory_path . '/file1.txt', 'Test Content 1');
        Storage::disk('public')->put($book->directory_path . '/file2.txt', 'Test Content 2');
        ...
        Storage::disk('public')->deleteDirectory($book->directory_path);
    }
}
