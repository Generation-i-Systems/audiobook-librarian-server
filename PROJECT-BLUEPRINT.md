...
- Book search uses one shared `/books` template for browsing and management; legacy `/admin/books` bookmarks redirect there. The `manage-books` permission layers management actions, missing-directory visibility, full catalog inclusion, every legacy sort option, result counts, and a duration-bearing list view onto the browsing views; full administrators additionally retain the AI Library Assistant. Search (web book list, `GET /books`, `GET /books/search`, and `GET /api/v1/books/search`) supports inline name tokens (`genre:Fantasy`, `author:"Name"`, `series:Name`, `tag:Name`) alongside the existing id tokens (`genreId:N`, `authorId:N`, `seriesId:N`, `bookId:N`), including combined positive genre and negative tag filters such as `genreId:1 tag:-spicy`. An opt-in `semantic=true` toggle ranks results by AI semantic similarity via the recommendation engine's embedding pipeline instead of exact/partial text match (falls back to normal search when no embedding provider is configured).
- All guards (`web`, `api`, `api_test`) authenticate as the Eloquent `App\Models\User` model: `DocumentUserProvider` retrieves/validates users through Eloquent directly instead of wrapping them in the legacy `DocumentstoreUser` value object. Previously `Auth::user()` returned `DocumentstoreUser` on session requests but `User` on bearer-token requests, so any code touching Eloquent relations (e.g. `UserLibraryController`'s queue/history pages) or non-whitelisted attributes crashed or silently read nulls in production while `actingAs()`-based tests passed. `tests/Feature/UserLibrarySessionAuthTest.php` locks the real session-login path in. `DocumentstoreUser` survives only as a lightweight DTO (console `ApiServiceClient`, legacy test fixtures).
...
- Users & access control is role/permission driven: roles are database-backed groups of permissions (`roles` + `role_permission`, seeded from the canonical `App\Models\Role` map) keyed by the `users.role` string; `User::hasPermission()` resolves the role bundle plus direct per-user grants (`permission_user`). The ordinary roles (`user`, `library-user`, `hybrid-user`) imply the standard library-management bundle (books/authors/genres/series/tags/badges); admin roles imply everything (string-based admin bypass retained as a safety net for permission keys added after seeding); `unverified` and the pure-listener `librivox-user` imply nothing. Enforcement stays in `permission:` middleware + dynamically defined Gates; the admin user edit page marks role-derived permissions and role dropdowns/validation all draw from the canonical role set. `GET /api/v1/user` and `GET /api/v1/me` expose `role` and the effective `permissions` list.
- Book form: multiple authors/series (autocomplete), Google Books autofill, genre selection, file uploads, and session-authenticated audio preview from persisted book directories
...
- `EmailOtpController`: passwordless email OTP/magic-link login — request/verify (API and web-session), magic-link landing page with `apiUrl`-aware deep links (`AppConnectLinks::magicPlayerDeepLink`/`magicLibraryDeepLink`/`androidMagicIntentLink`) so self-hosted installs route back to the issuing server; gated end-to-end by `MailConfiguration::isMailConfigured()`. `Admin\UserController`/`Api\AdminUserController` expose "send login email" and "generate login QR" for existing users from the admin UI. The login-QR URL embeds the account's username and the plaintext 6-digit code as query parameters (`.../auth/magic/<token>?username=<u>&otp=<code>`) alongside the one-time magic token, and the JSON response also returns `server_name`/`api_url`/`username`/`email`/`code`/`token`, so the same QR serves both a regular camera scanner (browser → magic landing → app deep link) and the Librarian app's connect-screen scanner (token redeemed directly via `POST /auth/otp/verify`).
