...
- Book CRUD (admin). Book detail views resolve the session-authenticated `DocumentstoreUser` wrapper to its Eloquent `User` model before calling tag/group services, which require persisted user relationships.
