...
| **Book contribution admin email** | `app/Services/BookContributionNotifier.php`, `app/Mail/BookContributionSubmittedMail.php` | Tests verify that each active admin recipient is selected and mailed through Laravel's mail fake, but real SMTP credentials, delivery, spam filtering, and the admin's ability to act on the email must be verified in production. |
...
- **Admin badge image upload** (`Admin\BadgeController::storeUploadedImage()`) — writes to
  ...
  `image_url` in production. A missing/broken symlink or permission mismatch on the real
