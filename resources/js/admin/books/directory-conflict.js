...
(function() {
    ...
    function initDirectoryConflictDetection() {
        ...
        const currentBookId = urlParts[urlParts.length - 2] === 'books' ? urlParts[urlParts.length - 1] : null;
        ...
        $('#conflict-move-other-btn').on('click', function() {
            ...
            alert('This feature is not yet implemented. Please choose another option.');
        });
    ...
    function checkDirectoryConflict(directoryPath, currentBookId, onSubmit) {
        ...
        if (!window.BOOK_FORM_ROUTES || !window.BOOK_FORM_ROUTES.checkDirectoryConflict) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.conflict) {
                    ...
                    if (onSubmit) {
                        $('#book-form').data('allowDirectoryMerge', true).submit();
                    }
                }
            },
        ...
        if (data.books && data.books.length > 0) {
            data.books.forEach(function(book) {
                ...
                $li.html(
                    ...
                    '<br><small class="text-muted ms-4">by ' + escapeHtml(book.author) + '</small>'
                );
                $bookList.append($li);
            });
        }
