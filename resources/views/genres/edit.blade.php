            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('genres.destroy', $genre['id']) }}" method="POST" class="mt-2">
                            @csrf
                            @method('DELETE')
                            @if($total > 0)
                                ...
                                <nav aria-label="Books pagination">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item {{ $currentPage <= 1 ? 'disabled' : '' }}">
                                            <a class="page-link" href="{{ route('genres.edit', ['genre' => $genre['id'], 'page' => $currentPage - 1]) }}">Previous</a>
                                        </li>

                                        @for($i = max(1, $currentPage - 2); $i <= min($totalPages, $currentPage + 2); $i++)
                                            <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                                                <a class="page-link" href="{{ route('genres.edit', ['genre' => $genre['id'], 'page' => $i]) }}">{{ $i }}</a>
                                            </li>
                                        @endfor

                                        <li class="page-item {{ $currentPage >= $totalPages ? 'disabled' : '' }}">
                                            <a class="page-link" href="{{ route('genres.edit', ['genre' => $genre['id'], 'page' => $currentPage + 1]) }}">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            @endif
