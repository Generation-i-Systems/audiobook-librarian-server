            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="border-bottom pb-3 mb-3">
                        <h5 class="text-dark">Orphaned: <code class="bg-light text-dark">{{ $match['orphaned_path'] }}</code></h5>
                        <small class="text-dark">Size: {{ number_format($match['orphaned_size'] / 1024 / 1024, 2) }} MB</small>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-8">
                                            ...
                                            <p class="mb-1 text-dark"><small>Authors: {{ implode(', ', $suggestion['authors']) }}</small></p>
                                            <p class="mb-1 text-dark"><small>Series: {{ implode(', ', $suggestion['series']) }}</small></p>
                                            <p class="mb-1 text-dark"><small>Expected Path: <code class="bg-light text-dark">{{ $suggestion['book_path'] }}</code></small></p>
                                            <div class="mt-2">
                                        ...
                                        <div class="col-md-4 text-end">
                                            ...
                                            <div class="d-grid gap-2">
                                                @if($hasPermission)
                                                <form action="{{ route('admin.directory-validation.rename') }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="book_id" value="{{ $suggestion['book_id'] }}">
                                                    <input type="hidden" name="orphaned_path" value="{{ $match['orphaned_path'] }}">
                                                ...
                                                @else
                                                <button type="button" class="btn btn-sm btn-primary w-100 mb-2" disabled title="No write permission">
                                                    Rename to Match
                                                </button>
                                                @endif
                                                ...
                                                @if($hasPermission)
                                                <form action="{{ route('admin.directory-validation.delete-orphan') }}" method="POST" class="w-100">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="hidden" name="orphaned_path" value="{{ $orphanData['full_path'] }}">
                                                    <button type="submit" class="btn btn-sm btn-danger w-100" 
                                                            onclick="return confirm('Delete this directory and all its contents? This cannot be undone!')">
                                                        Delete Directory
                                                    </button>
                                                </form>
                                                @else
                                                <button type="button" class="btn btn-sm btn-danger w-100" disabled title="No write permission">
                                                    Delete Directory
                                                </button>
                                                @endif
                                                @endif
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('admin.directory-validation.delete-book') }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <input type="hidden" name="book_id" value="{{ $book->id }}">
                                            <button type="submit" class="btn btn-sm btn-danger" 
                                                    onclick="return confirm('Delete this book entry?')">
                                                Delete
                                            </button>
                                        </form>
                                    </td>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="text-muted mb-0">No orphaned directories found. Click "Rescan Now" above to scan for orphaned directories.</p>
                    @endif
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <br><small class="text-danger">⚠️ Permission denied - cannot rename or delete</small>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @else
                                        <button type="button" class="btn btn-sm btn-warning" disabled title="No write permission">
                                            Rename
                                        </button>
                                        @endif
                                        
                                        @if(($orphan['parent_writable'] ?? true))
                                        <form action="{{ route('admin.directory-validation.delete-orphan') }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <input type="hidden" name="orphaned_path" value="{{ $orphan['full_path'] }}">
                                            <button type="submit" class="btn btn-sm btn-danger" 
                                                    onclick="return confirm('Delete this directory and all its contents? This cannot be undone!')">
                                                Delete
                                            </button>
                                        ...
                                        @else
                                        <button type="button" class="btn btn-sm btn-danger" disabled title="No write permission">
                                            Delete
                                        </button>
                                        @endif
                                    </td>
            ...
            <form action="{{ route('admin.directory-validation.rename-orphan') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">Edit the full path as needed. Path is relative to book storage root.</div>
                    </div>
                </div>
