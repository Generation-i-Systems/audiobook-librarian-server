(function (window, $) {
    ...
    if (!$) {
        console.error("import_directory.js requires jQuery");
        return;
    }
    ...
    function renderDirectoryBrowser(data, path) {
        ...
        directories.forEach((dir) => {
            ...
            const addUrl = "/admin/books/create?path=" + encodeURIComponent(dir.path || "");
        ...
        files.forEach((file) => {
            ...
            rows.push(
                "<tr class=\"file-row\">" +
                    "<td>" + safeName + "</td>" +
                    "<td>File</td>" +
                    "<td><span class=\"text-muted\">" + escapeHtml(size) + "</span></td>" +
                    "</tr>"
            );
        });

        if (!rows.length) {
            rows.push('<tr><td colspan="3" class="text-muted">No items found.</td></tr>');
        }
    ...
    function loadDirectory(path = "", updateFilter = true, updateHistory = false, callback) {
    ...
    function handleLetterFilterClick(event) {
    ...
    function handleBulkImportClick(event) {
        ...
        const currentPath = state.currentPath || new URL(window.location.href).searchParams.get("path") || "";
        ...
        $.ajax({
            ...
            success(response) {
                if (statusEl.length) {
                    statusEl.text(response?.message || "Bulk import started successfully");
                }
            },
            error(xhr) {
                if (statusEl.length) {
    ...
    function updateBookAction(bookId, editUrl) {
        if (!bookId) {
            return;
        }
        let $button = $(
    ...
    function registerEventHandlers() {
        $(document).on("click", ".directory-link", handleDirectoryClick);
        $(document).on("click", ".breadcrumb-link", handleBreadcrumbClick);
        $(document).on("click", ".add-book-btn", handleAddBookClick);
        $("#bulk-import-btn").on("click", handleBulkImportClick);
        $("#letter-filter").on("click", "button", handleLetterFilterClick);
        $("#search-filter").on("input", handleSearchInput);
    }
