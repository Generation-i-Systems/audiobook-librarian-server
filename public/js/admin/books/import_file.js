...
(function ($) {
    ...
    window.initImportFileBrowser = function (rootSelector, options = {}) {
        ...
        function init() {
            ...
            $.getJSON(ajaxRootsUrl, function (data) {
                ...
                for (const root of data) {
                    $select.append('<option value="' + root.value + '">' + root.label + '</option>');
                }
        ...
        function renderDirectoryList(data) {
            ...
            $list.find('.list-group-item').on('click', function() {
                ...
                if (type === 'parent') {
                    ...
                    loadDirectory(currentPath ? currentPath + '/' + name : name);
                } else if (type === 'file') {
                    // Select file
        ...
        $root.off('click', '#import-move-btn').on('click', '#import-move-btn', function () {
            ...
            // Log DOM state
            console.log('Active item:', $root.find('.list-group-item.active').length ? 'found' : 'not found');
            console.log('Active item data:', {
                name: $root.find('.list-group-item.active').data('name'),
                type: $root.find('.list-group-item.active').data('type')
            });

            // Log path input value
            console.log('Path input value:', $root.find('#import-path-input').val());
                ...
                const extractedDirectoryPath = $summary.data('directoryPath');
                ...
                const author = extractedAuthor || meta.author || 'Unknown Author';
                const series = (extractedSeries !== undefined && extractedSeries !== null) ? extractedSeries : (meta.series || '');
                const genrePath = extractedGenrePath || extractedGenre || 'Other';
            ...
            console.log('Path information:', {
                ...
                'Input field path': $root.find('#import-path-input').val() || '',
                'currentPath variable': currentPath || '',
                'Active item name': $root.find('.list-group-item.active').data('name') || ''
            });

            // CRITICAL CHECK: Verify directoryPath is set correctly and not blank
            console.log('directoryPath is set to:', meta['directoryPath']);

            // If directoryPath is blank, abort the request and show error
            if (!meta['directoryPath'] || meta['directoryPath'] === '') {
                $summary.html('<div class="alert alert-danger">Cannot move: Current directory path is blank. Please navigate to a valid directory first.</div>');
                console.groupEnd();
                return; // Stop execution here
            }

            // Debug AJAX headers
            console.log('AJAX headers:', {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')});
            ...
            console.log('Final request data:', JSON.parse(JSON.stringify(meta)));
            console.groupEnd();
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success) {
                        ...
                        $summary.html('<div class="alert alert-success mb-3 cursor-pointer" style="cursor: pointer;" title="Click to Create book record">Moved to: <code>' + (data.newPath || '-') + '</code> <small>(click to import)</small></div>' +
                ...
                error: function (xhr, status, error) {
                    ...
                    if (xhr.status === 403) {
                        errorMsg = 'Permission denied (403 Forbidden). Please check your session or refresh the page.';
                    ...
                    $summary.html('<div class="alert alert-danger">' + errorMsg + '</div>');

                    // Try to refresh the CSRF token
                    $.get(window.location.href, function(data) {
                        const newToken = $(data).filter('meta[name="csrf-token"]').attr('content');
                        if (newToken) {
                ...
                $root.find('#import-metadata-summary').html('<div class="alert alert-danger">Error processing move request: ' + error.message + '</div>');
                console.groupEnd();
            }
        });
        ...
        $(document).on('click', '#import-metadata-summary .alert-success', function(e) {
        ...
        function handleImportPrefill($btn) {
            ...
            console.log('Extracting metadata from:', $root.find('#import-metadata-summary').siblings('ul').html());

            // Extract metadata from the list items in the metadata section
            // First try the metadata list in the summary section
            const $metadataList = $root.find('#import-metadata-summary').siblings('ul').find('li');
            console.log('Found metadata items in summary:', $metadataList.length);
            ...
            if ($metadataList.length > 0) {
                $metadataList.each(function () {
                    ...
                    const value = $item.contents().filter(function () { return this.nodeType === 3; }).text().trim();

                    console.log('Metadata item from summary:', label, '=', value);
                ...
                const $browserMetadata = $root.find('.file-metadata-section').find('li');
                console.log('Found metadata items in browser:', $browserMetadata.length);
                ...
                $browserMetadata.each(function () {
                    ...
                    console.log('Metadata item from browser:', label, '=', value);
            ...
            if (!meta.title && $root.find('.list-group-item.active').length) {
                const activeItemName = $root.find('.list-group-item.active').text().trim();
            ...
            if (!meta.directorypath && $root.find('.list-group-item.active').length) {
                meta.directorypath = $root.find('.list-group-item.active').data('path') ||
                                    $root.find('.list-group-item.active').data('name') ||
                                    currentPath + '/' + $root.find('.list-group-item.active').text().trim();
                console.log('Set directoryPath from active item:', meta.directorypath);
            }

            // Check if we have moved the files (look for success message with newPath)
            const $successMessage = $root.find('#import-metadata-summary .alert-success');
            let importPath, importRoot;

            if ($successMessage.length && $successMessage.text().includes('Moved to:')) {
                // Extract the path from the success message
                const newPathMatch = $successMessage.find('code').text();
                console.log('Found moved path in success message:', newPathMatch);
                ...
                importPath = currentPath ? currentPath + '/' + $root.find('.list-group-item.active').data('name') : $root.find('.list-group-item.active').data('name');
                console.log('Using original path:', importPath);
            }

            // Set import path and root in metadata
            meta.import_path = importPath;
            meta.import_type = $root.find('.list-group-item.active').data('type') || 'dir';
            ...
            if (!meta.author && meta.import_path) {
                ...
                if (pathParts.length > 2) {
                    ...
                    if (possibleAuthor && possibleAuthor !== 'books' && possibleAuthor !== 'audiobooks') {
            ...
            if (!meta.genre && meta.import_path) {
                ...
                if (pathParts.length > 3) {
                    ...
                    if (possibleGenre && possibleGenre !== 'books' && possibleGenre !== 'audiobooks') {
            ...
            if (meta.directoryPath) {
                ...
                console.log('Using import_path as directory_path:', bookData.directory_path);
            }
            ...
            if (meta.series) {
                ...
                if (pathParts.length > 1) {
                    ...
                    const seriesMatch = dirName.match(/(?:book|vol(?:ume)?|part|#)\s*(\d+)/i);
                    if (seriesMatch) {
                        // Try to extract series name from directory name
                        let seriesName = dirName.replace(/(?:book|vol(?:ume)?|part|#)\s*\d+/i, '').trim();
                        seriesName = seriesName.replace(/[\(\)\[\]\{\}]/g, '').trim(); // Remove brackets
                        ...
                        if (seriesName) {
                            ...
                            console.log('Extracted series from path:', bookData.series);
                        }
                    }
                }
            }
            ...
            console.log('Redirecting to book creation form with data:', bookData);
            ...
            $root.find('#import-result').html('<div class="alert alert-info">Redirecting to book creation form...</div>');
...
$(document).ready(function() {
    ...
    if ($containers.length > 0) {
        ...
        if (window.initImportFileBrowser && window.initImportFileBrowser.loadRoots) {
            window.initImportFileBrowser.loadRoots();
        }
    } else if ($('#import-file-browser-root').length > 0) {
        // Fallback to the default selector
        const instance = window.initImportFileBrowser('#import-file-browser-root');
        if (instance && instance.loadRoots) {
            instance.loadRoots();
        }
    } else {
